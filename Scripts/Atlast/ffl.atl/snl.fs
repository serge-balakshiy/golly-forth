\ ==============================================================================
\
\           snl - the generic single linked list in the fflв
\
...
: snl-(free)   ( xt scl -- = Free the nodes from the heap using xt )
	swap
	BEGIN
		over snl.remove-first \ execute \ snl.remove-first
		nil<>?      \ while remove first node <> nil do
	WHILE
...
: snl-new      ( -- snl = Create a new snl list on the heap )
  snl% allocate  exp-invalid-memory throw?  dup snl-init
;
...
: snl+offset   ( n1 n2 -- n3 = Determine the offset n3 from the index n1 and length n2, incl. validation )
	tuck
	index2offset                    \ convert to offset
	dup rot 0
	within                     \ check outside 0..length-1
	exp-index-out-of-range throw?     \ raise exception
;

7 step
: snl-node  ( n snl -- snn1 | nil  snn2 | nil )
	( = Get the nth node in the list, return this node snn2 and its leading node snn1 )
  nil -rot                   \ prv  = nil
  snl-first@                 \ cur  = first
  swap                       \ S: prv cur off
  BEGIN
	  2dup 0> swap
	  nil<> AND   \ while n>0 and cur<> nil do
  WHILE
    1- >r                    \  off--
    nip                      \  prv = cur
...
: snl-remove-first   ( snl -- snn | nil = Remove the first node from the list, return the removed node )
  dup snl-first@ \ "snl-remove-first: " 502 steps
  ...
  THEN
;
\ ' snl-remove-first to snl.remove-first
...
: snl-remove-after   ( snn1 snl -- snn2 | nil )
  ...
  swap
  dup nil= exp-invalid-parameters throw?
...
64 step
: snl-merge-nodes  ( psize p^ snl -- 0 p^ | psize nil )
	( == Move all nodes from p^ in the list )
	>r
	BEGIN                           \ S:psize p^
		2dup nil<> AND
